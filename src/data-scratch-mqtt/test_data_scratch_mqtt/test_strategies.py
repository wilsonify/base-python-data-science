...
def test_sqrt(mqtt_client):
    body_dict = dict(strategy="sqrt", x=10)
...
def test_strength(mqtt_client):
    body_dict = dict(strategy="strength", expected=10, actual=6)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_echo(mqtt_client):
    body_dict = dict(strategy="echo", expected=10, actual=6)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_difference_quotient(mqtt_client):
    body_dict = dict(strategy="difference_quotient", x=5, h=1)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_distance(mqtt_client, vec1, vec2, expected):
    body_dict = dict(strategy="distance", v=vec1, w=vec2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_estimate_gradient(mqtt_client):
    body_dict = dict(strategy="estimate_gradient", v=[10.0, 2.0], h=1)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_in_random_order(mqtt_client):
    body_dict = dict(strategy="in_random_order", data=[1, 2, 3, 4, 5, 6])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_maximize_batch(mqtt_client):
    body_dict = dict(strategy="maximize_batch", x=[[1, 2, 3], [2, 3, 4], [5, 6, 7]])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_maximize_stochastic(mqtt_client):
    body_dict = dict(
        strategy="maximize_stochastic",
...
def test_minimize_batch(mqtt_client):
    body_dict = dict(
        strategy="minimize_batch",
        x=[[1, 49, 4, 0], [1, 41, 9, 0], [1, 40, 8, 0], [1, 25, 6, 0], [1, 21, 1, 0], [1, 21, 0, 0], [1, 19, 3, 0],
           [1, 19, 0, 0], [1, 18, 9, 0], [1, 18, 8, 0]],
        y=[68.77, 51.25, 52.08, 38.36, 44.54, 57.13, 51.4, 41.42, 31.22, 34.76, ]
    )
...
def test_partial_difference_quotient(mqtt_client):
    body_dict = dict(
        strategy="partial_difference_quotient",
        v=[10.0, 2.0],
        i=0,
        h=1
    )
...
def test_dot(mqtt_client, vec1, vec2, expected):
    body_dict = dict(strategy="dot", v=vec1, w=vec2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_get_column(mqtt_client, mat1, col, expected):
    body_dict = dict(strategy="get_column", mat=mat1, col=col)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_get_row(mqtt_client, mat1, row, expected):
    body_dict = dict(strategy="get_row", mat=mat1, row=row)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_magnitude(mqtt_client, vec1, expected):
    body_dict = dict(strategy="magnitude", v=vec1)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_matrix_add(mqtt_client, mat1, mat2, expected):
    body_dict = dict(strategy="matrix_add", mat1=mat1, mat2=mat2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_scalar_multiply(mqtt_client, v, c, expected):
    body_dict = dict(strategy="scalar_multiply", v=v, c=c)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_shape(mqtt_client):
    body_dict = dict(strategy="shape", mat=[[1, 2, 3], [3, 4, 5], [6, 7, 8]])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_squared_distance(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="squared_distance", v=v1, w=v2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_sum_of_squares(mqtt_client, x, expected):
    body_dict = dict(strategy="sum_of_squares", v=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_vector_add(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="vector_add", v=v1, w=v2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_vector_mean(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="vector_mean", vectors=[v1, v2])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_vector_subtract(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="vector_subtract", v=v1, w=v2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_vector_sum(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="vector_sum", vectors=[v1, v2])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_accuracy(mqtt_client, tp, fp, fn, tn, expected):
    body_dict = dict(strategy="accuracy", tp=tp, fp=fp, fn=fn, tn=tn)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_f1_score(mqtt_client, tp, fp, fn, tn, expected):
    body_dict = dict(strategy="f1_score", tp=tp, fp=fp, fn=fn, tn=tn)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_precision(mqtt_client, tp, fp, fn, tn, expected):
    body_dict = dict(strategy="precision", tp=tp, fp=fp, fn=fn, tn=tn)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_recall(mqtt_client, tp, fp, fn, tn, expected):
    body_dict = dict(strategy="recall", tp=tp, fp=fp, fn=fn, tn=tn)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_split_data(mqtt_client):
    body_dict = dict(
        strategy="split_data",
        mat=[[1, 2, 3, 4], [3, 4, 5, 6], [4, 5, 6, 7], [5, 6, 7, 8]],
        p=0.5
    )
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
...
def test_train_test_split(mqtt_client):
    body_dict = dict(
...
def test_bernoulli_trial(mqtt_client):
    body_dict = dict(strategy="bernoulli_trial", p=0.5)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_binomial(mqtt_client):
    body_dict = dict(strategy="binomial", n=10, p=0.6)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_inverse_normal_cdf(mqtt_client, p, mu, sigma, expected):
    body_dict = dict(strategy="inverse_normal_cdf", p=p, mu=mu, sigma=sigma)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_normal_cdf(mqtt_client, x, mu, sigma, expected):
    body_dict = dict(strategy="normal_cdf", x=x, mu=mu, sigma=sigma)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_normal_pdf(mqtt_client, x, mu, sigma, expected):
    body_dict = dict(strategy="normal_pdf", x=x, mu=mu, sigma=sigma)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_random_kid(mqtt_client):
    body_dict = dict(strategy="random_kid")
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_uniform_cdf(mqtt_client, x, expected):
    body_dict = dict(strategy="uniform_cdf", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_uniform_pdf(mqtt_client, x, expected):
    body_dict = dict(strategy="uniform_pdf", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_bucketize(mqtt_client, point, bucket_size, expected):
    body_dict = dict(strategy="bucketize", point=point, bucket_size=bucket_size)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_correlation(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="correlation", x=v1, y=v2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_covariance(mqtt_client, v1, v2, expected):
    body_dict = dict(strategy="covariance", x=v1, y=v2)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_data_range(mqtt_client, x, expected):
    body_dict = dict(strategy="data_range", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_de_mean(mqtt_client, x, expected):
    body_dict = dict(strategy="de_mean", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_interquartile_range(mqtt_client, x, expected):
    body_dict = dict(strategy="interquartile_range", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_mean(mqtt_client, x, expected):
    body_dict = dict(strategy="mean", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_median(mqtt_client, x, expected):
    body_dict = dict(strategy="median", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_mode(mqtt_client, x, expected):
    body_dict = dict(strategy="mode", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_quantile(mqtt_client, p, x, expected):
    body_dict = dict(strategy="quantile", x=x, p=p)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_standard_deviation(mqtt_client, x, expected):
    body_dict = dict(strategy="standard_deviation", x=x)
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
...
def test_variance(mqtt_client):
    body_dict = dict(strategy="variance", x=[1, 2, 3, 4])
    body_str = json.dumps(body_dict)
    body_bytes = body_str.encode("utf-8")
    mqtt_client.publish(
        topic=MQTT_TOPIC,
